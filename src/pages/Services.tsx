...
const services = [
  {
    ...
    description:
      'Replacements, repairs, and inspections using materials from GAF, CertainTeed, Atlas, and Owens Corning. Certified installers and manufacturer warranties.',
  },
  {
    icon: PanelLeft,
    title: 'Siding',
...
const certs = [
  { icon: Trophy, title: 'Atlas Pro+ Diamond' },
  { icon: Shield, title: 'GAF Certified Weather Stopper' },
  { icon: Award, title: 'CertainTeed Master Applicator' },
]
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              ...
              <ul className="space-y-3">
                {whyUs.map((item) => (
                  <li key={item} className="flex items-center gap-3 text-text-secondary">
                    <div className="w-2 h-2 rounded-full bg-guardian-blue shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
            ...
            >
              {certs.map((cert, i) => (
                ...
                >
                  <cert.icon className="w-8 h-8 text-guardian-blue shrink-0" />
